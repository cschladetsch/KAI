...
Pointer<Continuation> Compiler::Compile(Registry &R, const String &text, RhoStructure st) const
{
	if (text.empty())
		return Object();
...
Pointer<Continuation> Compiler::CompileFile(Registry &R, const String &fileName, RhoStructure st) const
{
	std::shared_ptr<RhoLang> p = std::make_shared<RhoLang>(R);
