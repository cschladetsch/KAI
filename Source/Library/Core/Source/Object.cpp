...
ObjectConstructParams::ObjectConstructParams(Registry *R, const ClassBase *C, Handle H, Constness K) : registry(R), class_base(C), handle(H), constness(K)
{
}
...
Object::Object(const ObjectConstructParams &P) : registry(P.registry), class_base(P.class_base), handle(P.handle)
{
}
...
void Object::SetParentHandle(Handle H)
{
	GetStorageBase().SetParentHandle(H);
}
...
{
	if (Exists())
		GetStorageBase().SetConst();
}

void Object::SetManaged(bool B) const
{
	if (Exists())
		GetStorageBase().SetManaged(B);
}

void Object::SetClean(bool B) const
{
	GetStorageBase().SetClean(B);
}
...
{
	return KAI_NAMESPACE(GetParent)(*this);
}
...
Object Object::Get(const Label &L) const
{
	return GetStorageBase().Get(L);
}

bool Object::Has(const Label &L) const
{
	return GetStorageBase().Has(L);
}

void Object::Remove(const Label &L) const
{
	GetStorageBase().Remove(L);
}

void Object::Detach(const Object &Q) const
{
	GetStorageBase().Detach(Q);
}
...
{
	return GetStorageBase().GetLabel();
}
...
Object::ChildProxy::ChildProxy(Object const &Q, Label const &L) : registry(Q.GetRegistry()), handle(Q.GetHandle()), label(L)
{
}
