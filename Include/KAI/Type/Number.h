...
struct NumberValue
{
	...
	NumberValue(NumberEnum V = NumberEnum::None) : value(V) { }
	NumberValue(int N) : value(static_cast<NumberEnum>(N)) { }
	friend bool operator<(NumberEnum A, NumberEnum B) { return A < B; }
	friend bool operator==(NumberEnum A, NumberEnum B) { return A == B; }
	friend bool operator!=(NumberEnum A, NumberEnum B) { return A != B; }
