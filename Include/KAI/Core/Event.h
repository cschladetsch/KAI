...
//
//        Bar bar;
//        Event<int, string> event;                // create a new event which
//        takes int and string args event += foo;                            //
//        add a callback to foo function event += pair(&bar, &Bar::spam); // add
//        a callback to a bound method event(42, "hello"); // invoke the event.
//        foo and bar.spam will both be called event -= foo; // remove foo
//        function from delegate list event(12, ", world"); // now just bar.spam
//        will be called
//
// NOTES
...
template <class T0 = event_detail::Null, class T1 = event_detail::Null,
          class T2 = event_detail::Null>
struct Event
    : event_detail::EventType<
          event_detail::Arity<T0, T1, T2>::Value>::template Create<T0, T1, T2> {
};

// template <class T0 = event_detail::Null, class T1 = event_detail::Null, class
// T2 = event_detail::Null> struct SystemEvent :
// event_detail_sys::EventType<event_detail::Arity<T0, T1, T2>::Value>::template
// Create<T0, T1, T2> { };
