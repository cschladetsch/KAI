...
struct CreateFunction<2> {
    ...
    struct Given : Delegate<2>::Given<T0, T1> {
        ...
        Function fun;
        Given(Function F) : fun(F) {}
        void Invoke(T0 t0, T1 t1) { fun(t0, t1); }
    };
};
...
struct CreateFunction<3> {
    ...
    struct Given : Delegate<3>::Given<T0, T1, T2> {
        ...
        Function fun;
        Given(Function F) : fun(F) {}
        void Invoke(T0 t0, T1 t1, T2 t2) { fun(t0, t1, t2); }
    };
};
...
struct CreateMethod<0> {
    ...
    struct Given {
        ...
        struct Type<true, Class> : Delegate<0>::Given<> {
            ...
            void Invoke() {
                if (object.Exists()) (Deref<Class>(object).*method)();
            }
        };
    };
};
...
struct CreateMethod<1> {
    ...
    struct Given {
        ...
        struct Type : Delegate<1>::Given<T0> {
            ...
            Class *object;
            Type(Class *Q, Method M) : object(Q), method(M) {}
            void Invoke(T0 t0) { (object->*method)(t0); }
        };
        template <class Class>
        struct Type<true, Class> : Delegate<1>::Given<T0> {
            ...
            Type(Object const &Q, Method M) : object(Q), method(M) {}
            void Invoke(T0 t0) {
                if (object.Exists()) (Deref<Class>(object).*method)(t0);
            }
        };
    };
};
...
struct CreateMethod<2> {
    ...
    struct Given {
        ...
        struct Type : Delegate<2>::Given<T0, T1> {
            ...
            Class *object;
            Type(Class *Q, Method M) : object(Q), method(M) {}
            void Invoke(T0 t0, T1 t1) { (object->*method)(t0, t1); }
        };
        template <class Class>
        struct Type<true, Class> : Delegate<2>::Given<T0, T1> {
            ...
            void Invoke(T0 t0, T1 t1) {
                if (object.Exists()) (Deref<Class>(object).*method)(t0, t1);
            }
        };
    };
};
...
struct CreateMethod<3> {
    ...
    struct Given {
        ...
        struct Type : Delegate<3>::Given<T0, T1, T2> {
            ...
            Class *object;
            Type(Class *Q, Method M) : object(Q), method(M) {}
            void Invoke(T0 t0, T1 t1, T2 t2) { (object->*method)(t0, t1, t2); }
        };
        template <class Class>
        struct Type<true, Class> : Delegate<3>::Given<T0, T1, T2> {
            ...
            void Invoke(T0 t0, T1 t1, T2 t2) {
                if (object.Exists()) (Deref<Class>(object).*method)(t0, t1, t2);
            }
        };
    };
};
...
struct DelegateType {
    ...
    friend bool operator==(DelegateType A, DelegateType B) {
...
struct AddInvoker<2> : InvokerBase<2> {
    ...
    struct Given {
        ...
        void operator()(T0 t0, T1 t1) {
            typename Sinks::const_iterator A = sinks.begin(), B = sinks.end();
            for (; A != B; ++A) A->second->Invoke(t0, t1);
        }
    };
};
...
struct AddInvoker<3> : InvokerBase<3> {
    ...
    struct Given {
        ...
        void operator()(T0 t0, T1 t1, T2 t2) {
            typename Sinks::const_iterator A = sinks.begin(), B = sinks.end();
            for (; A != B; ++A) A->second->Invoke(t0, t1, t2);
        }
    };
};

template <class C, bool>
struct MethodObjectBaseType {
    typedef C *Type;
};
template <class C>
struct MethodObjectBaseType<C, true> {
    typedef Object Type;
};

template <class T>
struct IsStorageType {
    enum { Value = false };
};
template <>
struct IsStorageType<StorageBase> {
    enum { Value = true };
};
...
struct EventType {
    ...
    template <class T0, class T1, class T2>
    struct Create
        : AddInvoker<N>::template Given<
              T0, T1, T2>  // add the correct operator() method to the type
    {
        ~Create() { Clear(); }
        ...
        void Clear() {
            typename Sinks::const_iterator A = Parent::sinks.begin(),
                                           B = Parent::sinks.end();
            for (; A != B; ++A) delete A->second;
            Parent::sinks.clear();
        }

        /// Create the function delegate type
        typedef typename CreateFunction<N>::template Given<T0, T1, T2>
            FunctionDelegate;
        typedef typename FunctionDelegate::Function Function;

        /// Create the method delegate type. This is a structure as the method
        /// also needs a class type whereas a function does not
        template <class C, bool system_type = false>
        struct MethodObject : CreateMethod<N>::template Given<
                                  T0, T1, T2>::template Type<system_type, C> {
            typedef typename CreateMethod<N>::template Given<
                T0, T1, T2>::template Type<system_type, C>
                Parent;
            typedef typename Parent::Method Method;

            MethodObject(typename MethodObjectBaseType<C, system_type>::Type Q,
                         Method M)
                : Parent(Q, M) {}
        };
        ...
        void Add(Function fun) {
            Parent::sinks.push_back(std::make_pair(DelegateType::Function,
                                                   new FunctionDelegate(fun)));
        }

        /// Remove an existing function delegate
        void Remove(Function fun) {
            typename Sinks::iterator A = Parent::sinks.begin(),
                                     B = Parent::sinks.end();
            for (; A != B; ++A) {
                if (A->first != DelegateType::Function) continue;
                FunctionDelegate *F =
                    reinterpret_cast<FunctionDelegate *>(A->second);
                if (F->fun != fun) continue;
                delete A->second;
        ...
        // static MethodObject<C> Method(B *Q, typename
        // MethodObject<B,C>::Method M)
        //{
        ...
        void AddMethod(C *base, typename MethodObject<C>::Method method) {
            Parent::sinks.push_back(std::make_pair(
                DelegateType::Method, new MethodObject<C>(base, method)));
        }
        template <class C>
        void AddMethod(Object object, typename MethodObject<C>::Method method) {
            Parent::sinks.push_back(
                std::make_pair(DelegateType::Method,
                               new MethodObject<C, true>(object, method)));
        }
        template <class C>
        void AddMethod(std::pair<C *, typename MethodObject<C>::Method> bound) {
            Parent::sinks.push_back(
                std::make_pair(DelegateType::Method,
                               new MethodObject<C>(bound.first, bound.second)));
        }

        // template <class C>
        // void AddMethod2(StorageBase *base, typename MethodObject<C>::Method
        // method)
        //{
        //     //sinks.push_back(std::make_pair(DelegateType::Method, new
        //     MethodObject<C>(base, method)));
        // }
        ...
        //{
        //     //*this += std::make_pair(&chained,
        //     &MethodObject<EventType<1>::Create<U0> >::operator());
        // }
        ...
        void Remove(MethodObject<C, S> bound) {
            typename Sinks::iterator A = Parent::sinks.begin(),
                                     B = Parent::sinks.end();
            for (; A != B; ++A) {
                if (A->first != DelegateType::Method) continue;
                MethodObject<C, S> *M =
                    reinterpret_cast<MethodObject<C, S> *>(A->second);
