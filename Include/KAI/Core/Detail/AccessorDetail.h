...
namespace property_detail
{
    ...
    struct CommonBase : Base
    {
        ...
        CommonBase(Field G, Label L, bool B, MemberCreateParams::Enum createParams)
            : Base(L, Type::Traits<C>::Number, Type::Traits<T>::Number, B,createParams)
              , field(G) { }
    };
    ...
    struct NonSystemProperty : CommonBase<Base,C,T,F>
    {
        ...
        NonSystemProperty(Field G, Label const &L, MemberCreateParams::Enum createParams)
    ...
    // common functionality for accessors and mutators of _properties which are system types.
    template <class Base, class C, class T, class F>
    struct SystemProperty : CommonBase<Base,C,T,F>
    {
        ...
        SystemProperty(Field field, Label const &L, MemberCreateParams::Enum createParams)
    ...
    struct Accessor<Class, C, true, T, S> : SystemProperty<AccessorBase, C, T, S (Class::*)>
    {
    ...
    struct Mutator<K,Class,true,T,S> : SystemProperty<MutatorBase, K, T, S (Class::*)>
    {
        typedef SystemProperty<MutatorBase, K, T, S (Class::*)> Parent;
        typedef typename Parent::Field Field;
        using Parent::field;

        Mutator(Field field, Label const &label, MemberCreateParams::Enum  createParams)
        ...
        void SetValue(Object const &Q, Object const &V) const
        {
            ...
            object.Assign(object.GetStorageBase(), V.GetStorageBase());    // use the class system to perform the assignment
        }
    };

    // make an accessor (read-only) property
    // Class is the type of the containing structure
    // Class is the type of the structure which defines the member. this may be the same as Class.
